...
public class HistoryPresenterTest extends BaseTest {
    ...
    public void shouldShowNotificationOnGetLogError() throws Exception {
        ...
        verify(notificationManager).notify(eq("error"), eq(FAIL), eq(NOT_EMERGE_MODE));
    }
    ...
    public void shouldShowCompareWhenOneFileChangedInCurrentRevision() throws Exception {
        ...
        ShowFileContentResponse showFileContentResponse2 = mock(ShowFileContentResponse.class);
        when(showFileContentResponse1.getContent()).thenReturn("content1");
        when(showFileContentResponse2.getContent()).thenReturn("content2");
